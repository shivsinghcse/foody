            ...
            {items.map((item) => {
                return (
                    <div
                        ...
                        className="border-b-1 md:border-b-2 border-gray-300 pt-2 md:pt-4 pb-4 md:pb-5 px-3 flex justify-between"
                    >
                        <div className="overflow-hidden w-4/5">
                            <p>
                                {item?.card?.info?.itemAttribute?.vegClassifier ===
                                'VEG' ? (
                                    <span className="border-1 md:border-2 border-green-500 pb-[0.7px] rounded text-[8px]  md:text-xs">
                                        🟢
                                    </span>
                                ) : (
                                    <span className="border-1 md:border-2 border-red-500 pb-[0.7px] rounded text-[8px]  md:text-xs">
                                        🔴
                                    </span>
                                )}
                            ...
                            <p className="text-sm md:text-md md:font-medium mr-1 md:mr-6 leading-4 md:leading-5 text-gray-600 cursor-pointer  ">
                                {item?.card?.info?.description?.length > 100 ? item?.card?.info?.description?.slice(0, 96)+'...' : item?.card?.info?.description}
                            </p>
                        </div>
                        ...
                        <div className="flex items-end justify-center  relative my-8">
                            ...
                            />
                            <button
                                className="border-[1px] border-gray-300 bg-white px-2 md:px-6 md:py-2 py-1 md:rounded-lg rounded-md text-sm md:text-md font-medium md:font-extrabold text-green-600 mx-auto absolute -bottom-5 z-1 shadow-md cursor-pointer"
                                onClick={() => {
                                    handleAddItem(item);
                                }}
                            >
